...
def apply_conflict(
    ...
    """Resolve two disagreeing values without inventing a third.

    The more authoritative source wins, the disagreement is recorded, and both values are
    preserved. Equal authority leaves the field flagged CONFLICTING for human review.
    """
    if existing == incoming or incoming is None:
        return existing
    if existing is None:
        return incoming
    record = {
        "field": field,
        "kept": str(existing),
        "rejected": str(incoming),
        "existing_trust": existing_trust,
        "incoming_trust": incoming_trust,
        "source_url": source_url,
    }
    if incoming_trust < existing_trust:
        record["kept"], record["rejected"] = str(incoming), str(existing)
        draft.conflicts.append(record)
        return incoming
    draft.conflicts.append(record)
    if incoming_trust == existing_trust and field in draft.evidence:
        draft.evidence[field].status = "CONFLICTING"
    return existing
